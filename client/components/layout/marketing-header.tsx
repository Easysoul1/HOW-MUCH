    ...
    <header className="sticky top-0 z-50 w-full section-divider bg-white/90 backdrop-blur-md supports-[backdrop-filter]:bg-white/85 dark:bg-dark/90 dark:supports-[backdrop-filter]:bg-dark/85">
      <div className="container flex h-14 md:h-16 items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" asChild className="text-muted dark:text-muted-foreground">
          ...
          <Button variant="ghost" size="icon" className="md:hidden text-muted dark:text-muted-foreground" onClick={() => setMobileOpen(!mobileOpen)}>
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={cn(
                  ...
                  pathname === link.href ? "bg-primary/5 text-primary dark:bg-accent/10 dark:text-white" : "text-muted dark:text-muted-foreground hover:bg-light-panel dark:hover:bg-dark-elevated hover:text-primary dark:hover:text-white"
                )}
              >
                {link.label}
              </Link>
