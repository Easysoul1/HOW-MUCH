  ...
  return (
    <motion.form
      ...
      className={cn(
        ...
        isHero && "max-w-3xl rounded-2xl border-2 border-light-border bg-white p-2 shadow-lg",
        isSticky && "sticky top-16 z-40 rounded-xl border border-light-border bg-white/95 p-2 shadow",
        className
      )}
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2 sm:flex-nowrap">
        <Select value={weight} onValueChange={setWeight}>
          <SelectTrigger className={cn("w-full sm:w-[100px]", isHero && "h-12 border-0 bg-light-panel")}>
        ...
        <Select value={location} onValueChange={setLocation}>
          <SelectTrigger className={cn("w-full sm:w-[120px]", isHero && "h-12 border-0 bg-light-panel")}>
      ...
      {isHero && focused && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="absolute left-0 right-0 top-full z-10 mt-2 rounded-lg border border-light-border bg-white p-2 shadow-lg"
        >
          ...
          {POPULAR_ITEMS.map((item) => (
            <button
              key={item}
              type="button"
              className="block w-full rounded-md px-2 py-2 text-left text-sm hover:bg-light-panel"
              onClick={() => {
                setQuery(item);
                setFocused(false);
              }}
